...
(function($){
	...
	function drawSin(xOffset,color1,color2){
	    ...
	    /**绘制百分比文本**/
	    //修改第58行: ~~config.nowRange → config.targetRange,抓targetRange才會顯示小數點後的數字 Michelle 20190418
	    //~~config.nowRange: 他只會顯示整數 ex:46.75% → 46%
		if (!config.isLoading) {
			ctx.save();
			...
			ctx.fillText(config.targetRange + '%', config.circle_config.r, config.circle_config.r + size / 2);
			ctx.restore();
		}
		return canvas;
	}
